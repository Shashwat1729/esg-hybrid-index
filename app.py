...
META_PATH = ROOT / "data" / "processed" / "cleaning_metadata.json"
...
try:
    _mtime = date.fromtimestamp(DATA_PATH.stat().st_mtime).isoformat()
except Exception:
    _mtime = date.today().isoformat()

REPO_URL = "https://github.com/Shashwat1729/esg-hybrid-index"
PAPER_URL = "https://github.com/Shashwat1729/esg-hybrid-index#paper"
...
""")
    ...
    gr.Markdown(
        f"""
        ---
        **Disclaimer — Academic Research Demonstration, Not Financial Advice.** Past cross-sectional patterns do not predict future performance. No guarantee of future returns. Consult a licensed financial advisor before making investment decisions. All return figures are trailing momentum proxies, not realized returns.

        *Multi-Factor ESG Investment Index* — Dataset: **{META_STR}** · Generated **{_mtime}** · Scores 50 ± σ (clip [0,100]) · [GitHub]({REPO_URL}) · Methodology: Index Methodology & About tabs.
        """
    )
